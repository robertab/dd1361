...
class Evol a  where
  ...
  distanceMatrix seqlist@(x:xs) =
    map (\y -> (name x,name y,distance x y)) seqlist ++ distanceMatrix xs
...
makeProfileMatrix sl = res
  where t = seqType (head sl) -- Första Molseq i [MolSeq]
        defaults =
          if (t == DNA)
...
profileDistance prof1 prof2 =
  sum $ zipWith (\x y -> abs (x - y)) matrix1 matrix2
  where matrix1 = [snd x | xs <- matrix prof1, x <- xs]
